...
static void tg3_mdio_config_5785(struct tg3 *tp)
{
	...
	phydev = tp->mdio_bus->phy_mask[tp->phy_addr];
...
static void tg3_setup_flow_control(struct tg3 *tp, u32 lcladv, u32 rmtadv)
{
	...
	if (tg3_flag(tp, USE_PHYLIB))
		autoneg = tp->mdio_bus->phy_mask[tp->phy_addr]->autoneg;
	else
...
static void tg3_adjust_link(struct net_device *dev)
{
	...
	struct phy_device *phydev = tp->mdio_bus->phy_mask[tp->phy_addr];
...
static int tg3_phy_init(struct tg3 *tp)
{
	...
	case PHY_INTERFACE_MODE_MII:
		...
		break;
	default:
		phy_disconnect(tp->mdio_bus->phy_mask[tp->phy_addr]);
		return -EINVAL;
	}
...
static void tg3_phy_stop(struct tg3 *tp)
{
	...
	phy_stop(tp->mdio_bus->phy_mask[tp->phy_addr]);
}
...
static int tg3_nway_reset(struct net_device *dev)
{
	...
	if (tg3_flag(tp, USE_PHYLIB)) {
		if (!(tp->phy_flags & TG3_PHYFLG_IS_CONNECTED))
			return -EAGAIN;
		r = phy_start_aneg(tp->mdio_bus->phy_mask[tp->phy_addr]);
	} else {
		u32 bmcr;
